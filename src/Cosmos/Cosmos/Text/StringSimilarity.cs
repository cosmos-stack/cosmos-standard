...
namespace Cosmos.Text
{
    public static class StringSimilarity
    {
        ...
        /// <returns></returns>
        public static double EvaluateSimilarity(string text, string toCheck, double similarityMinimal)
        {
        ...
        /// <returns></returns>
        public static double EvaluateSimilarity(string text, string toCheck, double similarityMinimal, int diffFound)
        {
            ...
            var resConCarAwayInText2 = EvaluateSimilarity(text.Substring(posDifference + 1), toCheck.Substring(posDifference), similarityMinimal, differences);
            //Consider if the dif is a missing character in text1
            var resConCarAwayInText1 = EvaluateSimilarity(text.Substring(posDifference), toCheck.Substring(posDifference + 1), similarityMinimal, differences);
            //Consider if the dif is a changed character
            var resConCarCharacter = EvaluateSimilarity(text.Substring(posDifference + 1), toCheck.Substring(posDifference + 1), similarityMinimal, differences);
        ...
        /// <param name="toCheck"></param>
        /// <returns></returns>
        public static StringSimilarityTypes EvaluateTypeSimilarity(this string text, string toCheck)
        {
            //ignore spaces
            text = text.RemoveChars(' ');
            toCheck = toCheck.RemoveChars(' ');
            ...
            if (text.Length != toCheck.Length)
            {
                if (text.Length > toCheck.Length)
                    ...
                    portionToCheck = toCheck.Substring(0, text.Length);
                if (portionText.EqualsIgnoreCase(portionToCheck))
                    return (text.Length > toCheck.Length ? StringSimilarityTypes.MayorLong : StringSimilarityTypes.MinorLong);
            }
