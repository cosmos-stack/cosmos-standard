...
namespace CosmosStack.Exceptions
{
    ...
    public class Failure<T> : Try<T>
    {
        ...
        {
            return RecoverWith(ex => CosmosStack.Exceptions.Try.LiftValue(recoverFunction(ex)));
        }

        /// <inheritdoc />
        public override Try<T> Recover(Func<Exception, string, T> recoverFunction)
        {
            return RecoverWith(ex => CosmosStack.Exceptions.Try.LiftValue(recoverFunction(ex.InnerException, ex.Cause)));
        }
        ...
        internal override Try<TResult> Bind<TResult>(Func<T, Try<TResult>> bind) => CosmosStack.Exceptions.Try.LiftException<TResult>(Exception);
