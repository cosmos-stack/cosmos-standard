...
public static partial class Types
{
            ...
            && isOptions switch
               {
                   ...
                   TypeIsOptions.IgnoreNullable => TypeConv.GetNonNullableType(type)?.IsEnum ?? false,
                   _ => type.IsEnum
               };
    }
    ...
    public static bool IsStructType(Type? type, TypeIsOptions isOptions = TypeIsOptions.Default)
    {
        ...
        bool __check(Type? yourType) => (yourType?.IsValueType ?? false) && !yourType.IsEnum && !yourType.IsPrimitive;
        return type is not null
            && isOptions switch
               {
